...
public class DefaultMQAdminExtImpl implements MQAdminExt, MQAdminExtInner {
    ...
    public int wipeWritePermOfBroker(final String namesrvAddr, String brokerName)
        ...
        return this.mqClientInstance.getMQClientAPIImpl().wipeWritePermOfBroker(namesrvAddr, brokerName,
            3000);
    }
    ...
    public List<RollbackStats> resetOffsetByTimestampOld(String consumerGroup, String topic, long timestamp,
        ...
        for (BrokerData bd : topicRouteData.getBrokerDatas()) {
            ...
            if (addr != null) {
                ...
                for (Map.Entry<MessageQueue, OffsetWrapper> entry : consumeStats.getOffsetTable()
                    .entrySet()) {
                ...
                if (!hasConsumed) {
                    HashMap<MessageQueue, TopicOffset> topicStatus = this.mqClientInstance
                        .getMQClientAPIImpl().getTopicStatsInfo(addr, topic, 3000).getOffsetTable();
    ...
    public void createOrUpdateOrderConf(String key, String value, boolean isCluster)
        ...
        if (isCluster) {
            this.mqClientInstance.getMQClientAPIImpl()
                .putKVConfigValue(NamesrvUtil.NAMESPACE_ORDER_TOPIC_CONFIG, key, value, 3000);
        }
        else {
            String oldOrderConfs = null;
            try {
                oldOrderConfs = this.mqClientInstance.getMQClientAPIImpl()
                    .getKVConfigValue(NamesrvUtil.NAMESPACE_ORDER_TOPIC_CONFIG, key, 3000);
            }
            catch (Exception e) {
                e.printStackTrace();
            }
