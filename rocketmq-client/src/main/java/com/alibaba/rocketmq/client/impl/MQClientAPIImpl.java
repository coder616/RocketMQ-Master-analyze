...
public class MQClientAPIImpl {
    ...
    public void createSubscriptionGroup(final String addr, final SubscriptionGroupConfig config,
        ...
        if (!UtilAll.isBlank(projectGroupPrefix)) {
            config.setGroupName(
                VirtualEnvUtil.buildWithProjectGroup(config.getGroupName(), projectGroupPrefix));
        }
    ...
    // 是否开启发送消息优化（减少网络中传输的字节数，将普通字节用单个字母代替）
            ...
            public void operationComplete(ResponseFuture responseFuture) {
                ...
                if (response != null) {
                    ...
                    if (!responseFuture.isSendRequestOK()) {
                        ...
                        sendCallback.onException(new MQClientException(
    ...
    public MessageExt viewMessage(final String addr, final long phyoffset, final long timeoutMillis)
        ...
        case ResponseCode.SUCCESS: {
            ...
            if (!UtilAll.isBlank(projectGroupPrefix)) {
                messageExt
                    .setTopic(VirtualEnvUtil.clearProjectGroup(messageExt.getTopic(), projectGroupPrefix));
            }
            return messageExt;
        }
        default:
    ...
    public void updateConsumerOffsetOneway(final String addr,
            final UpdateConsumerOffsetRequestHeader requestHeader, final long timeoutMillis)
                    throws RemotingConnectException, RemotingTooMuchRequestException,
                    RemotingTimeoutException, RemotingSendRequestException, InterruptedException {
        if (!UtilAll.isBlank(projectGroupPrefix)) {
            requestHeader.setConsumerGroup(
                VirtualEnvUtil.buildWithProjectGroup(requestHeader.getConsumerGroup(), projectGroupPrefix));
            requestHeader
     ...
     * @throws InterruptedException
     */
    public void unregisterClient(final String addr, final String clientID, final String producerGroup,
    ...
    public Set<MessageQueue> lockBatchMQ(final String addr, final LockBatchRequestBody requestBody,
            final long timeoutMillis) throws RemotingException, MQBrokerException, InterruptedException {
        if (!UtilAll.isBlank(projectGroupPrefix)) {
            requestBody.setConsumerGroup(
                (VirtualEnvUtil.buildWithProjectGroup(requestBody.getConsumerGroup(), projectGroupPrefix)));
            Set<MessageQueue> messageQueues = requestBody.getMqSet();
            for (MessageQueue messageQueue : messageQueues) {
                messageQueue.setTopic(
                    VirtualEnvUtil.buildWithProjectGroup(messageQueue.getTopic(), projectGroupPrefix));
            }
        }
        ...
        case ResponseCode.SUCCESS: {
            ...
            if (!UtilAll.isBlank(projectGroupPrefix)) {
                for (MessageQueue messageQueue : messageQueues) {
                    messageQueue.setTopic(
                        VirtualEnvUtil.clearProjectGroup(messageQueue.getTopic(), projectGroupPrefix));
                }
            }
     ...
     * @throws InterruptedException
     */
    public void unlockBatchMQ(final String addr, final UnlockBatchRequestBody requestBody,
    ...
    public TopicRouteData getDefaultTopicRouteInfoFromNameServer(final String topic, final long timeoutMillis)
        ...
        GetRouteInfoRequestHeader requestHeader = new GetRouteInfoRequestHeader();// netty请求头
        requestHeader.setTopic(topic);
        // 设置请求类型
        RemotingCommand request =
    ...
    /**
     * 从nameserver根据Topic获取Broker Name、队列数(包含读队列与写队列)
     * 
     * @param topic
     * @param timeoutMillis
     * @return
    ...
    /**
     * 从nameserver 通过本地IP获取 ProjectGroup
     * 
     * @param ip
     * @param timeoutMillis
     * @return
     ...
     * @throws InterruptedException
     */
    public String getProjectGroupByIp(String ip, final long timeoutMillis)
            throws RemotingException, MQClientException, InterruptedException {
    ...
    /**
     * 从nameserver通过namespace获取所有key-value 配置
     * 
     * @param namespace
     * @param timeoutMillis
     * @return
    ...
    public ConsumerRunningInfo getConsumerRunningInfo(final String addr, String consumerGroup,
            String clientId, boolean jstack, final long timeoutMillis)
                    throws RemotingException, MQClientException, InterruptedException {
